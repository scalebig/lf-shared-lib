...
function LfNum (rawNum, options) {
  ...
  if (_.isString(rawNum)) {
    let rawNumCheck = rawNum.trim()
    if (rawNum === '' || (rawNum.length > 0 && (rawNum[0] === '[' || rawNum[0] === '{'))) {
      this.num = null
    } else {
...
LfNum.prototype.isSigFigCount = function (compareTo) {
  return new SigFigures(this.value).sigFigures() === parseInt(compareTo)
}

LfNum.prototype.toSigFigCount = function (targetCount) {
  let value = this.strNum
  return mathjs.format(lfNum(value).toFloat(), {precision: targetCount, lowerExp: -7, upperExp: 7})
  // let parseNum = {}
...
LfNum.prototype.isInToleranceRange = function (expected, lowerMultiplier, upperMultiplier, sigFigures, decimalPlaces) {
  let val = this.toFloat()
  let expectedVal = lfNum(expected).toFloat()

  if (upperMultiplier === null || _.isUndefined(upperMultiplier)) {
    upperMultiplier = 1
  }
  if (lowerMultiplier === null || _.isUndefined(lowerMultiplier)) {
    lowerMultiplier = 1
  }
  let lowerBound = expectedVal * lowerMultiplier
  let upperBound = expectedVal * upperMultiplier
  console.log('before sig fig')
  console.log({
    actual: val,
    expected,
    expectedVal,
  ...
  if (sigFigures) {
    lowerBound = lfNum(lowerBound).toSigFigCount(sigFigures, true)
    upperBound = lfNum(upperBound).toSigFigCount(sigFigures, true)
  } else if (!_.isNullish(decimalPlaces) && decimalPlaces !== '') {
    lowerBound = lfNum(lowerBound).round(lfNum(decimalPlaces).num)
    upperBound = lfNum(upperBound).round(lfNum(decimalPlaces).num)
  }

  console.log('after sig fig')
  console.log({
    actual: val,
    expected,
    expectedVal,
  ...
  // if expecting a negative number, the multiplied lower bound results in a greater value
  // than the upper bound, so if we want a value between them, we need to flip the signs
  if (expected < 0) {
