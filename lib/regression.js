...
const methods = {
  linear (data, options) {
    ...
    const predictX = y => ([
      round(y, options.precision),
      round((y - intercept) / gradient, options.precision)]
    )
  ...
  linearRTO (data, options) {
    ...
    const predictX = y => ([
      round(y, options.precision),
      round(y / gradient, options.precision)]
    )

    const points = data.map(point => predict(point[0]))
    const r2RTO = round((sum[3] ** 2 / sum[2] / sum[4]), options.precision)
